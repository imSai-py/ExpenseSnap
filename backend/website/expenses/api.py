...
def get_upload_folder():
    ...
    # static_folder is set to '../static' relative to website/, so it's backend/static/
    if current_app.static_folder:
        upload_folder = os.path.join(current_app.static_folder, 'uploads', 'profiles')
    else:
        # Fallback: go up one level from website/ to backend/, then into static/
...
def create_expense():
    ...
    try:
        ...
        if new_expense.type == 'expense':
            try:
                from ..notifications import check_budget_and_notify
